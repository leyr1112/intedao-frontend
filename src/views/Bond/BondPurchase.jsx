...
function BondPurchase({ bond, slippage, recipientAddress }) {
  ...
  const setMax = () => {
    ...
    if (bond.maxBondPrice * bond.bondPrice < Number(bond.balance)) {
      ...
      maxQ = bond.balance;
      // if (bond.name == "Inte_usdc_lp" || bond.isFour) {
              ...
              <div className="help-text">
                <em>
                  <Typography variant="body1" align="center" color="textSecondary">
                    First time bonding <b>{displayName}</b>? <br /> Please approve Inte Dao to use your{" "}
                    <b>{displayName}</b> for bonding.
                  </Typography>
                </em>
              </div>
            ) : (
              <FormControl className="inte-input" variant="outlined" color="primary" fullWidth>
